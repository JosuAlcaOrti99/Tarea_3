...
Datos_Policiales %>%
  select(Delito, Fecha, Victima, Provincia, Canton, Distrito) %>%                     #carga las columnas que selecciono
  mutate(Fecha = as.Date(Fecha, format = "%d/%m/%Y"))%>%                                  # cambia el formato de la columna a fecha 
    datatable(
    colnames = c("Delito", "Fecha", "Víctima", "Edad", "Género", "Provincia", "Cantón" ), # le cambia el nombre a las columnas
    options = list(
    pageLength = 20,# dice cuantos datos se pueden visualizar
    language = list(url = '//cdn.datatables.net/plug-ins/1.10.11/i18n/Spanish.json')      # cambia el lenguaje del datatable para que los controles esten en español
  ))# genera el data table 
```
